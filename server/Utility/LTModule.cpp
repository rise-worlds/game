...
//以下代码段用于查找内存泄露
#if defined(ARKDEBUG_NEW)
#define new ARKDEBUG_NEW
#undef THIS_FILE
...
void CInterfaceDatabase::Add(IBase *api, const char *implemented_class_name, int32 interface_version)
{
	...
	if (name_mgr == NULL)
	{
		...
		name_mgr = new CInterfaceNameMgr(implemented_class_name, interface_version);
...
void CInterfaceDatabase::Add(CAPIHolderBase *holder)
{
	...
	{
		//create a name mgr for this interface name.
		name_mgr = new CInterfaceNameMgr(holder->InterfaceName(), holder->Version());
...
void CInterfaceDatabase::Add(CInterfaceChooser *chooser)
{
	...
	{
		//create a name mgr for this interface name.
		name_mgr = new CInterfaceNameMgr(chooser->InterfaceName(), chooser->InterfaceVersion());
...
void CInterfaceDatabase::TransferFrom(CInterfaceDatabase *other)
{
	...
	for (int32 i = other->interfaces->Num() - 1; i >= 0; i--)
	{
		...
		// 2004.04.09 修改内存泄漏
		if (other_mgr->IsEmpty() == true)
		{
			//remove it from the array.
			other->interfaces->remove(other_mgr);
...
#endif
SetMasterDatabase(CInterfaceDatabase *new_database)
{
	//check parameters
...
int BindModule(const char *pModuleName)
{
	...
	if (hInstance == NULL)
	{
		...
		strm << "LoadLibrary failed,can't load module:" << pModuleName << std::ends;
		GenErr(strm.str());
	}
