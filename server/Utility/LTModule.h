...
#include <dlfcn.h>
/*
 * Summ: Q: How to build "CModule" module under linux platform?
 *		 A: gcc -rdynamic -o foo foo.c -ldl
 *		 Q: How to build other module as "DLL" under linux platform?
 *		 A: with the following g++ options: "-Wall -O2 -fpic -shared -o"
 */
#define		LoadLibrary(szModuleFileName)		dlopen(szModuleFileName, RTLD_NOW)
//#define		LoadLibrary(szModuleFileName)		dlopen(szModuleFileName, RTLD_LAZY)

#define		FreeLibrary(hModule)			dlclose(hModule)
#define		GetProcAddress(hModule, szRoutineName)	dlsym(hModule, szRoutineName)
#endif
...
#define ARKCOMMON_API COMMON_API
#define GenErr

#ifndef _WIN32
typedef void* 	HMODULE;
typedef void*	HINSTANCE;
HMODULE GetModuleHandle(const char* szModuleFileName);
#endif
...
class ARKCOMMON_API CAPIHolderBase
{
public:
...
#ifndef _WIN32
extern "C" void SetMasterDatabase(CInterfaceDatabase *master_database);
typedef void (*TSetMasterFn)(CInterfaceDatabase *master_database);
CInterfaceDatabase *GetMasterDatabase();
int ARKCOMMON_API BindModule(const char *pModuleName);
#else
//
...
template <class C>
class ARKCOMMON_API CPointerTrackedTarget
{
public:
...
class ARKCOMMON_API CInterfaceDatabase
{
public:
	CInterfaceDatabase();
	virtual ~CInterfaceDatabase();
...
#if !defined _WIN32
// flying modified:
// In linux, simply call abort, brings the same effect of "__asm int 3".
#define BREAK1() abort()
#else
