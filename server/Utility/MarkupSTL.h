...
#ifndef __MARKUPSTL_H__
#define __MARKUPSTL_H__
...
public:
	...
	std::string GetElemContent() const
	{
	...
	bool AddElem(const char* szName, const char* szData = NULL, int nFlags = 0)
	{
		return x_AddElem(szName, szData, nFlags);
	};
	bool InsertElem(const char* szName, const char* szData = NULL, int nFlags = 0)
	{
	...
	bool InsertChildElem(const char* szName, const char* szData = NULL, int nFlags = 0)
	{
		return x_AddElem(szName, szData, nFlags | MNF_INSERT | MNF_CHILD);
	};
	bool AddElem(const char* szName, int nValue, int nFlags = 0)
	{
		return x_AddElem(szName, nValue, nFlags);
	};
	bool InsertElem(const char* szName, int nValue, int nFlags = 0)
	{
	...
	bool InsertChildElem(const char* szName, int nValue, int nFlags = 0)
	{
	...
	bool AddNode(int nType, const char* szText)
	{
		return x_AddNode(nType, szText, 0);
	};
	bool InsertNode(int nType, const char* szText)
	{
	...
	bool SetChildData(const char* szData, int nFlags = 0)
	{
	...
	bool SetElemContent(const char* szContent)
	{
...
protected:
	...
	struct ElemPos
	{
		...
		{
			return (nTagLengths & EP_STMASK);
		};
		void SetStartTagLen(int n)
		{
			nTagLengths = (nTagLengths & ~EP_STMASK) + n;
		};
		...
		int Level() const
		{
			return nFlags & EP_LEVMASK;
		};
		void SetLevel(int nLev)
		{
			nFlags = (nFlags & ~EP_LEVMASK) | nLev;
		};
	...
	struct TokenPos
	{
		TokenPos(const char* sz, int n)
		{
			Clear();
			szDoc = sz;
			nTokenFlags = n;
		};
		TokenPos(const std::string& str, int n)
		{
			Clear();
			szDoc = str.c_str();
			nTokenFlags = n;
		...
		bool Match(const std::string& strName)
		{
			return Match(strName.c_str());
		};
	...
	struct SavedPosMap
	{
		...
		void Release()
		{
			for (int n = 0; n < SPM_SIZE; ++n) if (pTable[n]) delete[] pTable[n];
			delete[] pTable;
		};
		void AllocMapTable()
		{
			pTable = new SavedPos*[SPM_SIZE];
			for (int n = 0; n < SPM_SIZE; ++n) pTable[n] = NULL;
		};
		int Hash(const char* szName)
		{
			int n = 0;
			while (*szName) n += *szName++;
			return n % SPM_SIZE;
		};
		SavedPos** pTable;
	};
	...
	struct PosArray
	{
		...
		void Release()
		{
			for (int n = 0; n < SegsUsed(); ++n) delete[](char*)pSegs[n];
			if (pSegs) delete[](char*)pSegs;
		};
		void Clear()
		{
			nSegs = 0;
	...
	protected:
		...
		{
			for (int n = 0; n < nSize; ++n) pNNew[n] = pN[n];
			if (pN) delete [] pN;
			pN = pNNew;
		};
		NodePos* pN;
