...
IoHandler::~IoHandler()
{
	...
	if (m_pWaitAcceptSessionList)	delete m_pWaitAcceptSessionList;
...
void				IoHandler::Init(AsioNetwork *pIOCPServer, LPIOHANDLER_DESC lpDesc)
{
	...
	m_pAcceptSessionPool	= new SessionPool(*m_IoServicePtr, lpDesc->nMaxAcceptSession + 2 * EXTRA_ACCEPTEX_NUM,
	        lpDesc->nSendBufferSize,
	        lpDesc->nRecvBufferSize,
	        lpDesc->nMaxPacketSize,
	        lpDesc->nTimeOut,
	        1,
	        true);

	m_pConnectSessionPool	= new SessionPool(*m_IoServicePtr, lpDesc->nMaxConnectSession,
	        lpDesc->nSendBufferSize,
	        lpDesc->nRecvBufferSize,
	        lpDesc->nMaxPacketSize,
	        lpDesc->nTimeOut,
	...
	for (int i = 0; i < int(lpDesc->nNumberOfIoThreads); i++)
	{
		boost::shared_ptr<boost::thread> thread(new boost::thread(
		        boost::bind(&boost::asio::io_service::run, m_IoServicePtr)));
...
bool				IoHandler::StartListen(char *pIP, unsigned short nPort)
{
	...
	for (int i = 0; i < EXTRA_ACCEPTEX_NUM; i++)
	{
		...
		m_AcceptorPtr->async_accept(pSession->m_Socket,
		                            boost::bind(&IoHandler::handle_accept, this, pSession,
...
#ifdef _DEBUG
	m_pWaitAcceptSessionList->Lock();
	for (SessionList::iterator it = m_pWaitAcceptSessionList->begin();
	        it != m_pWaitAcceptSessionList->end(); it++)
	{
		if (*it == pSession)
		{
...
unsigned int		IoHandler::Connect(NetObject *pNetworkObject, char *pszIP, unsigned short nPort)
{
...

	{
		...
		pSession->m_Socket.async_connect(endpoint,
		                                 /*m_Strand.wrap(*/
		                                 boost::bind(&IoHandler::handle_connect, this, pSession,
		                                         boost::asio::placeholders::error)/*)*/);
