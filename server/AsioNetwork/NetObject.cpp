...
NetObject::NetObject() { m_pSession = NULL; }
...
{
    ENTER_FUNCTION_FOXNET
    if (!m_pSession || m_pSession->HasDisconnectOrdered() || nSize == 0)
        return false;
...
{
    ENTER_FUNCTION_FOXNET
    assert(pNetworkObject != NULL && "NULL 目标 Redirect");
...
bool NetObject::OnPreSendPack(void* pMsg, int nSize, float fBufferRation, int nMsgLevel) { return true; }
