...
public:
    ...
    inline bool Write(PACKET_HEADER* pHeader, char* pMsg)
    {
        ...
        if (!m_pQueue->Enqueue((char*) pHeader, sizeof(PACKET_HEADER)))
            return false;
    ...
    {
        if (!m_pQueue->Enqueue(pMsg, nSize))
            return false;
        return true;
    }

    inline unsigned int GetLength() { return m_pQueue->GetLength(); }
    inline unsigned int GetSpace() { return m_pQueue->GetSpace(); }

    inline void GetDataWriteParam(char** ppWritePtr, int& nLen) { m_pQueue->GetSendWritePtr(ppWritePtr, nLen); }
